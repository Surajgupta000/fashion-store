...
import { ShieldCheck, Truck, MessageCircle, ArrowLeft, ShoppingBag } from "lucide-react";
...
export default function Checkout() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] pt-24 md:pt-36 pb-32 md:pb-20 px-4 md:px-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-20">
          ...
          {/* --- SECTION 1: SELECTION SUMMARY (First on Mobile) --- */}
          <div className="flex-1 order-1 lg:order-2">
            <div className="bg-white border border-gray-100 p-6 md:p-8 shadow-sm rounded-sm lg:sticky lg:top-32">
              <h3 className="font-serif italic text-xl md:text-2xl mb-6 border-b border-gray-50 pb-4 text-arinya-dark flex items-center gap-3">
                <ShoppingBag className="w-5 h-5 text-arinya-gold" /> Items in Bag
              </h3>
              
              <div className="max-h-[30vh] md:max-h-[40vh] overflow-y-auto custom-scrollbar pr-2 mb-6 space-y-4">
                {cart.map((item) => (
                  <div key={item.id} className="flex gap-4 group border-b border-gray-50 pb-4 last:border-0">
                    ...
                    <div className="flex flex-col justify-between py-1 flex-1">
                      <div>
                        ...
                        <p className="font-sans text-[9px] text-arinya-gray mt-1 uppercase tracking-widest font-medium">Qty: {item.qty}</p>
                      </div>
              ...
              {/* Pricing breakdown visible on Desktop/Large Tablet */}
              <div className="hidden lg:block border-t border-gray-100 pt-6 space-y-4">
                ...
                <button
                  onClick={handleOrder}
                  className="btn-accent w-full mt-6 py-5 text-[11px] uppercase tracking-[0.3em] font-bold shadow-xl text-[#0F0F0F]"
                >
          ...
          {/* --- SECTION 2: CONCIERGE DETAILS (Middle on Mobile) --- */}
          <div className="flex-[1.5] space-y-10 order-2 lg:order-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 py-10 border-t border-gray-100">
              <div className="flex flex-col items-center lg:items-start gap-3 text-arinya-dark text-center lg:text-left">
                ...
                <span className="font-sans text-[9px] uppercase tracking-[0.2em] font-bold">Quality Insured</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-3 text-arinya-dark text-center lg:text-left">
                <Truck className="w-6 h-6 text-arinya-gold" />
                <span className="font-sans text-[9px] uppercase tracking-[0.2em] font-bold">White-Glove Shipping</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-3 text-arinya-dark text-center lg:text-left">
                <MessageCircle className="w-6 h-6 text-arinya-gold" />
                <span className="font-sans text-[9px] uppercase tracking-[0.2em] font-bold">Personal Concierge</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-4 z-[50] shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex flex-col">
            <span className="font-serif italic text-lg text-arinya-dark">Total Amount</span>
            <span className="font-sans font-bold text-xl text-arinya-dark">₹{total.toLocaleString('en-IN')}</span>
          </div>
          <button
            onClick={handleOrder}
            className="btn-accent flex-1 py-4 text-[11px] uppercase tracking-[0.2em] font-bold text-[#0F0F0F]"
          >
            Confirm Order
          </button>
        </div>
